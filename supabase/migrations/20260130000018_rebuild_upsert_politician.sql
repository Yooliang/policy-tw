-- ============================================================
-- 徹底重建 upsert_politician 函數
-- ============================================================

-- 刪除所有可能存在的版本
DROP FUNCTION IF EXISTS upsert_politician(TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, INT, TEXT, TEXT, INT);
DROP FUNCTION IF EXISTS upsert_politician(TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, INT, TEXT, TEXT, TEXT, INT);
DROP FUNCTION IF EXISTS upsert_politician(TEXT);

-- 建立新版本
CREATE FUNCTION upsert_politician(
  p_name TEXT,
...
DECLARE
  ...
  v_region_id INTEGER;
  v_result TEXT;
  v_status politician_status;
  v_election_type election_type;
BEGIN
  -- Cast enums upfront
  v_status := p_status::politician_status;
  v_election_type := p_election_type::election_type;
  ...
  IF v_politician_id IS NOT NULL THEN
    -- Update existing politician (avoid COALESCE with enum, use direct assignment)
    UPDATE politicians SET
      party = p_party,
      status = v_status,
      election_type = v_election_type,
      position = CASE WHEN p_position = '' THEN position ELSE p_position END,
  ...
  ELSE
    ...
    VALUES (
      p_name, p_party, v_status, v_election_type,
