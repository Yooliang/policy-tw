-- 查核之前先確認「這是不是政見」，不要一上來就補欄位。
--
-- 2026-09-16 小良哥看 /policy/2db8cc91（蘇巧慧「母雞帶小雞 - 最強新北隊」）：
--   「它看起來不是政見，而是口號。查核那邊反而還把它列進去…甚至還為它去更新了資料。
--     不應該等到人工審查的時候才去按『這不是政見』；應該要在查核之前先確認這是不是政見、
--     是不是需要更新，而不是一上來就覺得這是一個政見、要去更新它的日期。」
--
-- 原本三種以政見為對象的自動缺口，敘述都預設「這一列是政見」，只問缺什麼欄位：
--   policy_source_missing   → 「請找到原始來源後補 source_url」
--   progress_stale          → 「請確認有沒有當選、承諾兌現了沒有」
--   policy_election_missing → 「請把 election_id 改成該選舉年份」
-- 代理照著做，就會替一句選戰口號補日期、補屆別。「這不是政見？」只存在於人工按按鈕那條路。
--
-- 改法：
--   1. 沒有出處的政見不再派「補出處」，改派 policy_validity：先判斷是不是政見，
--      不是就 removal（3 票），是才找出處補 source_url。連出處都沒有的那一列，
--      最該問的是真偽，不是欄位。
--   2. progress_stale 與 policy_election_missing 的敘述都以「先判斷它是不是政見」開頭，
--      並明講不是政見就提 removal、不要補欄位。
--   3. 判準與例子寫在任務敘述裡（「母雞帶小雞」「溫暖創新的某某市」），代理不必自己猜。
--
-- 只改任務敘述與型別，沒有動篩選條件，缺口總數不變（policy_source_missing 那一批
-- 換成同樣筆數的 policy_validity）。
