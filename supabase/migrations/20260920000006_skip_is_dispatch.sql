-- 「跳過」只是「這題我不答」（使用者 2026-09-20）：它跟派過一樣，把任務排到後面就好，不該按來源 IP 記 24 小時不派——
-- 那會讓同一台機器上其他代理也拿不到那幾筆（W-Policy 的取樣代理若用 skip，真代理就被連坐）。
-- 派工排序已是「最久沒派的優先」，skip 時把 last_dispatched_at 往後推即可；contribution_task_skips 只留作紀錄，不再拿來排除。
...
COMMENT ON FUNCTION contribution_auto_tasks IS
  '自動任務派工：合格判斷都在 LIMIT 之前（認領中／同 IP 或同代號交過／在途 ≥5／no_change 在途／冷卻）；排序＝（沒派過且 Jev 有答案）一次性插隊 → 最久沒派 → task_id；skip 不排除、只是派過';
