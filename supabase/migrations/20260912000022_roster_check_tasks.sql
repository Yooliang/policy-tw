-- 把名單清查接進自動缺口。
-- 條件：（縣市 × 選舉類型）從來沒清查過，或最後一次超過 recheck_days 天。
-- 查完就不再派，過了週期又自己出現——跟其他自動缺口同一套邏輯。
--
-- 同時順手修一件事：原本 (a)(c)(d) 沒有排除軟移除的政見，
-- 移除掉的政見會繼續產生「沒有出處」「進度停滯」的任務。
...
LANGUAGE sql STABLE AS $fn$
  ...
  all_tasks AS (
    ...
    UNION ALL
    -- (f) 名單清查：一個任務只查一個縣市的一種選舉，範圍小才好驗證
