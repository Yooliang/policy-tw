...
--   POST /functions/v1/fetch-cec-data {"queryName":"蔡易餘","electionId":2024}
--   → { candidacies: [{ election_id, election_result, birth_year, party, area, … }], tickets }
--
-- 四種任務的提示一起換：election_result_missing、progress_stale（競選承諾那支）、
-- profile_gap（出生年）、candidacy_source_missing。
-- 只改 hint_sources 文字，篩選條件與缺口數量不變。
