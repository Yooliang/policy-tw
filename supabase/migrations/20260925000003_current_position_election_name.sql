...
CREATE OR REPLACE FUNCTION current_position_missing(p TEXT) RETURNS BOOLEAN
LANGUAGE sql IMMUTABLE AS $$
  SELECT p IS NULL OR btrim(p) = '' OR p ~ '選舉$'
$$;
COMMENT ON FUNCTION current_position_missing IS '現職算缺：空的、或存的是選舉名稱（「111年直轄市議員選舉」這種，早期匯入填錯的）';
