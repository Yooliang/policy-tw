-- 登記截止後，「傳聞參選」要被確認下來。
--
-- 2026-09-17 小良哥：「傳聞的參選人在名單出來之後，他就應該要被確認下來」。
-- 實查：63 筆 2026 的參選紀錄還掛著 rumored／likely（57 筆縣市議員、6 筆縣市長），
-- 而登記 2026-09-04 就截止了。
--
-- 這條規則本來只寫在 roster_check 任務的敘述裡（「登記截止之後，還標著 rumored 的人
-- 只要不在登記名單上就是沒登記」），但名單清查一直清不動——全國性的結構化名單要等
-- 11-17 公告才有，代理只能一個縣市一個縣市看。結果這 63 筆就沒有人碰。
--
-- 改成獨立的缺口 candidate_status_stale：一筆一個任務、各自派工、各自結案，
-- 不必等整個縣市的名單清查完成。出口只有三條：registered、not_running、或查不到用 no_change。
--
-- 登記截止日存在 roster_check_scope 上（跟公告日 list_announced_on 同一張表），
-- 新增一屆選舉時 NOT NULL 會強迫填，不會有人忘了它而讓任務永遠不觸發。

ALTER TABLE roster_check_scope ADD COLUMN IF NOT EXISTS registration_closed_on DATE;

UPDATE roster_check_scope SET registration_closed_on = DATE '2026-09-04'
 WHERE election_id = 2026 AND registration_closed_on IS NULL;

ALTER TABLE roster_check_scope ALTER COLUMN registration_closed_on SET NOT NULL;

COMMENT ON COLUMN roster_check_scope.registration_closed_on IS '候選人登記截止日。過了這天，rumored／likely 的參選紀錄就該被確認成 registered 或 not_running';
...
LANGUAGE sql STABLE AS $fn$
  ...
  -- 公告才有），所以 63 筆就這樣掛著。2026-09-17 小良哥：「傳聞的參選人在名單出來之後，
