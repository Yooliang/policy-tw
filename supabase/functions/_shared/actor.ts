 ...
 *
 * 這一版（第 1 步）只認匿名。ditrust: 前綴**先擋下來、講清楚還沒開放**，不能當成一般代號收進去——
 * 收了就等於把序號存進 agent_name、印在貢獻榜上。等 DiTrust 的 verify 端點上線再接（第 3 步）。
 */
...
export function agentNameProblem(agentName: string): string | null {
  if (isDitrustLogin(agentName)) {
    return "ditrust:<序號> 的登入方式還沒開放，請先用一般代號（2～64 字，字母數字與 ._-）。序號不要填進其他欄位。";
  }
