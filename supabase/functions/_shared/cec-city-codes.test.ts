...
function codesInAdminPage(src: string, name: string): { prv: string; city: string } | null {
  const i = src.indexOf(`name: '${name}'`);
  if (i < 0) return null;
  const m = src.slice(Math.max(0, i - 80), i).match(/prvCode:\s*'(\d+)',\s*cityCode:\s*'(\d+)'/);
  return m ? { prv: m[1], city: m[2] } : null;
}
...
Deno.test("AdminScraper.vue 的縣市代碼：跟 fetch-cec-data 同一份", async () => {
  ...
  for (const c of CANONICAL) {
    const got = codesInAdminPage(src, c.name);
    assert(got, `AdminScraper.vue 找不到 ${c.name}`);
    assertEquals(got, { prv: c.prv, city: c.city }, `${c.name} 的代碼配錯了（兩份清單要一致）`);
  }
});
