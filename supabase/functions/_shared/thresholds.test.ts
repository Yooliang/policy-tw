...
Deno.test("來源等級門檻：一般資料 官方 2／媒體 2／社群 3／其他 3", () => {
  assertEquals(requiredAgree("policy", {}, [OFFICIAL]), 3, "官方原本 1 票，但 1 票配上換機器就能規避的自我投票檢查等於單人可寫入");
  assertEquals(requiredAgree("policy", {}, [MEDIA]), 3);
  ...
  assertEquals(consensusStatus(tally([{ verdict: "agree" }, { verdict: "agree" }, { verdict: "agree" }]), "pending", requiredAgree("policy", {}, [OFFICIAL])), "verified", "目標 3：三張 +1 上線");
  assertEquals(consensusStatus(tally([{ verdict: "agree" }, { verdict: "agree" }]), "pending", requiredAgree("policy", {}, [SOCIAL])), "pending", "兩張 +1 不夠 3");
});

Deno.test("來源等級門檻：加減參選人 官方 4／媒體 6／社群與其他 8；correction 改 candidate_status 同級", () => {
  assertEquals(requiredAgree("candidacy", { candidate_status: "registered" }, [OFFICIAL]), 3);
  assertEquals(requiredAgree("candidacy", { candidate_status: "registered" }, [MEDIA]), 3);
  assertEquals(requiredAgree("candidacy", { candidate_status: "withdrawn" }, [SOCIAL]), 3);
  assertEquals(requiredAgree("candidacy", {}, [OTHER]), 3);
  assertEquals(requiredAgree("correction", { field: "candidate_status" }, [MEDIA]), 3);
  // 2026-09-20：傳聞參選改成登記／不參選是一般級（2 票），登記→不參選才是加減參選人
  assertEquals(requiredAgree("correction", { changes: [{ field: "candidate_status", current_value: "rumored", correct_value: "registered" }] }, [OFFICIAL]), 3);
  assertEquals(requiredAgree("correction", { changes: [{ field: "candidate_status", current_value: "likely", correct_value: "not_running" }] }, [MEDIA]), 3);
  assertEquals(requiredAgree("correction", { changes: [{ field: "candidate_status", current_value: "registered", correct_value: "not_running" }] }, [OFFICIAL]), 3);
  assertEquals(requiredAgree("correction", { changes: [{ field: "candidate_status", current_value: "rumored", correct_value: "registered" }, { field: "candidate_status", current_value: "confirmed", correct_value: "not_running" }] }, [OFFICIAL]), 3, "混著一筆真的加減就走高風險");
  assertEquals(requiredAgree("correction", { field: "birth_year" }, [MEDIA]), 3, "一般欄位是一般資料");
...
Deno.test("來源等級門檻：task_suggestion／no_change 官方 1 其餘 2；adjudication 一律 3（2026-09-21 從 4 降）；多來源取最高等級", () => {
  assertEquals(requiredAgree("task_suggestion", {}, [OFFICIAL]), 2);
  ...
  assertEquals(requiredAgree("policy", {}, [OTHER, SOCIAL, MEDIA]), 3, "官方沒有、媒體有 → 媒體");
  assertEquals(requiredAgree("policy", {}, [OTHER, "https://www.ly.gov.tw/Pages/x"]), 3, "有一個官方就算官方");
...
Deno.test("question_answer 走一般資料門檻（沒有特例）：官方 2／媒體 2／社群 3／其他 3", () => {
  ...
  assertEquals(requiredAgree("question_answer", {}, [OFFICIAL]), 3);
  assertEquals(requiredAgree("question_answer", {}, [MEDIA]), 3);
...
Deno.test("同一個來源 IP 一筆貢獻只算一票：SQL 用 COUNT(DISTINCT verifier_ip_hash)，TS 的 tallyByIp 要一致", async () => {
  ...
  // 三台不同機器 → 三票，達目標 3（2026-09-21 一律 3；兩台不夠）
  const twoMachines = [
    { verdict: "agree" as const, verifier_ip_hash: "aaa" },
    { verdict: "agree" as const, verifier_ip_hash: "bbb" },
  ];
  assertEquals(tallyByIp(twoMachines).agree, 2);
  assertEquals(consensusStatus(tallyByIp(twoMachines), "pending", AGREE_THRESHOLDS.normal.official), "pending", "兩票不夠 3");
  const threeMachines = [...twoMachines, { verdict: "agree" as const, verifier_ip_hash: "ccc" }];
  assertEquals(consensusStatus(tallyByIp(threeMachines), "pending", AGREE_THRESHOLDS.normal.official), "verified");
...
Deno.test("補已投票選舉的結果：目標一律 3、不看來源等級；沒帶 politician_id 仍是加減參選人（風險分級留著給 Jev 用）", () => {
  ...
  for (const src of [OFFICIAL, MEDIA, SOCIAL, OTHER]) assertEquals(requiredAgree("candidacy", pastResult, [src]), 3, `來源 ${src} 也是 3（一律 3）`);
  assertEquals(consensusStatus(tally([{ verdict: "agree" }, { verdict: "agree" }, { verdict: "agree" }]), "pending", requiredAgree("candidacy", pastResult, [SOCIAL])), "verified");
...
Deno.test("只有網域的首頁降到最低等級，具體那一頁才算官方", () => {
  ...
  assertEquals(requiredAgree("policy", {}, ["https://bulletin.cec.gov.tw/?dir=x"]), 3);
});
