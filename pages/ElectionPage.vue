...
import { CANDIDATES, POLICIES, LOCATIONS, CATEGORIES, getElectionById } from '../constants'
import { PolicyStatus, ElectionType } from '../types'
import PolicyCard from '../components/PolicyCard.vue'
import CandidateGrid from './election/CandidateGrid.vue'
import CandidateDropdown from './election/CandidateDropdown.vue'
...
const viewMode = ref<'candidates' | 'pledges' | 'issues' | 'comparison'>('candidates')
...
const OTHER_LOCATIONS = LOCATIONS.filter(loc => !SIX_CAPITALS.includes(loc))
...
const electionCandidates = computed(() =>
  CANDIDATES.filter(c => c.electionIds?.includes(electionId.value))
)
const filteredCandidates = computed(() =>
  selectedRegion.value === 'All' ? electionCandidates.value : electionCandidates.value.filter(c => c.region === selectedRegion.value)
)

const mayorCandidates = computed(() => filteredCandidates.value.filter(c => c.electionType === ElectionType.MAYOR || !c.electionType))
const councilorCandidates = computed(() => filteredCandidates.value.filter(c => c.electionType === ElectionType.COUNCILOR))
const townshipMayorCandidates = computed(() => filteredCandidates.value.filter(c => c.electionType === ElectionType.TOWNSHIP_MAYOR))
const indigenousChiefCandidates = computed(() => filteredCandidates.value.filter(c => c.electionType === ElectionType.INDIGENOUS_DISTRICT_CHIEF))
const repCandidates = computed(() => filteredCandidates.value.filter(c => c.electionType === ElectionType.REPRESENTATIVE))
const indigenousRepCandidates = computed(() => filteredCandidates.value.filter(c => c.electionType === ElectionType.INDIGENOUS_DISTRICT_REP))
const chiefCandidates = computed(() => filteredCandidates.value.filter(c => c.electionType === ElectionType.CHIEF))

const electionCandidateIds = computed(() => new Set(electionCandidates.value.map(c => c.id)))

const allCampaignPolicies = computed(() =>
  POLICIES.filter(p => p.status === PolicyStatus.CAMPAIGN && electionCandidateIds.value.has(p.candidateId) && (selectedRegion.value === 'All' || CANDIDATES.find(c => c.id === p.candidateId)?.region === selectedRegion.value))
)
...
const candidateAId = ref('')
const candidateBId = ref('')

watch([() => selectedRegion.value, () => comparisonLevel.value], () => {
  if (comparisonPool.value.length > 0) {
    candidateAId.value = comparisonPool.value[0].id
    candidateBId.value = comparisonPool.value.length > 1 ? comparisonPool.value[1].id : comparisonPool.value[0].id
  } else {
    candidateAId.value = ''
    candidateBId.value = ''
  }
...
const candidateA = computed(() => CANDIDATES.find(c => c.id === candidateAId.value))
const candidateB = computed(() => CANDIDATES.find(c => c.id === candidateBId.value))

const getPledge = (cId: string, category: string) =>
  POLICIES.find(p => p.candidateId === cId && p.status === PolicyStatus.CAMPAIGN && (p.category === category || p.tags.includes(category)))
...
<template>
  <div v-if="election" class="bg-slate-50 min-h-screen">
    ...
    <Hero full-width>
      ...
      <div class="flex items-center justify-between gap-4">
        ...
        <div class="flex bg-slate-100 p-1 rounded-lg shrink-0 overflow-x-auto">
          <button @click="viewMode = 'candidates'" :class="`px-4 py-2 rounded-md text-sm font-bold flex items-center gap-2 transition-all whitespace-nowrap ${viewMode === 'candidates' ? 'bg-white text-navy-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`"><LayoutGrid :size="16" /> 候選人</button>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <!-- VIEW: Candidates -->
      <div v-if="viewMode === 'candidates'" class="animate-fade-in">
        <CandidateGrid :candidates="mayorCandidates" title="縣市長參選人"><template #icon><Flag class="text-red-500" /></template></CandidateGrid>
        <CandidateGrid :candidates="councilorCandidates" title="縣市議員參選人"><template #icon><Users class="text-blue-500" /></template></CandidateGrid>
        <CandidateGrid v-if="townshipMayorCandidates.length > 0" :candidates="townshipMayorCandidates" title="鄉鎮市長參選人"><template #icon><Building2 class="text-indigo-500" /></template></CandidateGrid>
        <CandidateGrid v-if="indigenousChiefCandidates.length > 0" :candidates="indigenousChiefCandidates" title="原住民區長參選人"><template #icon><Mountain class="text-emerald-600" /></template></CandidateGrid>
        <CandidateGrid v-if="repCandidates.length > 0" :candidates="repCandidates" title="鄉鎮市民代表參選人"><template #icon><Landmark class="text-green-500" /></template></CandidateGrid>
        <CandidateGrid v-if="indigenousRepCandidates.length > 0" :candidates="indigenousRepCandidates" title="原住民區代表參選人"><template #icon><MessageCircle class="text-teal-500" /></template></CandidateGrid>
        <CandidateGrid v-if="chiefCandidates.length > 0" :candidates="chiefCandidates" title="村里長參選人"><template #icon><MapPin class="text-amber-500" /></template></CandidateGrid>
      </div>
      ...
      <div v-if="viewMode === 'comparison'" class="animate-fade-in space-y-8">
        ...
        <div class="bg-amber-50 rounded-xl border border-amber-200 p-6 flex flex-col md:flex-row items-center justify-between gap-8">
          <CandidateDropdown v-model="candidateAId" label="候選人 A" ring-color="blue-500" :selected-region="selectedRegion" :comparison-pool="comparisonPool" />
          ...
          <CandidateDropdown v-model="candidateBId" label="候選人 B" ring-color="red-500" :selected-region="selectedRegion" :comparison-pool="comparisonPool" />
        </div>

        <div v-if="!candidateA || !candidateB || candidateAId === candidateBId" class="text-center py-20 text-slate-400 border border-dashed border-slate-300 rounded-xl">
          <Swords :size="48" class="mx-auto mb-4 opacity-50" />
          <p>請選擇兩位不同的候選人。</p>
        </div>
        <div v-else class="grid grid-cols-1 gap-6 text-left">
          <template v-for="category in CATEGORIES" :key="category">
            <div v-if="getPledge(candidateAId, category) || getPledge(candidateBId, category)" class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-slate-100">
                <div class="p-6 hover:bg-blue-50/20 transition-colors">
                  <div v-if="getPledge(candidateAId, category)" @click="router.push(`/policy/${getPledge(candidateAId, category)!.id}`)" class="cursor-pointer group">
                    <h4 class="font-bold text-lg text-navy-900 mb-2 group-hover:text-blue-600 transition-colors">{{ getPledge(candidateAId, category)!.title }}</h4>
                    <p class="text-slate-600 text-sm mb-4 line-clamp-3">{{ getPledge(candidateAId, category)!.description }}</p>
                  </div>
                  <span v-else class="text-slate-400 text-sm italic">未提出相關承諾</span>
                </div>
                <div class="p-6 hover:bg-red-50/20 transition-colors">
                  <div v-if="getPledge(candidateBId, category)" @click="router.push(`/policy/${getPledge(candidateBId, category)!.id}`)" class="cursor-pointer group">
                    <h4 class="font-bold text-lg text-navy-900 mb-2 group-hover:text-red-600 transition-colors">{{ getPledge(candidateBId, category)!.title }}</h4>
                    <p class="text-slate-600 text-sm mb-4 line-clamp-3">{{ getPledge(candidateBId, category)!.description }}</p>
                  </div>
                  <span v-else class="text-slate-400 text-sm italic">未提出相關承諾</span>
                </div>
              </div>
