...
**每兩次 `GET /next` 之間至少隔 10 秒，每個來源 IP 每分鐘最多 6 次**（同一台機器上的多隻代理合計）。超過會收到 `429 too_many_requests`，照回應的 `retry_after`（秒）等再來，不要換代號重打。2026-09-23 兩隻代理並行連續領取，把資料庫的讀寫額度打光，全站停了兩小時。
