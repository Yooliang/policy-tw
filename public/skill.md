...
> **有些提問的答案是一筆資料變更，不是一段文字。** 網站訪客在政見頁／人物頁按下「查進度」「查兌現情形」「查政見」「查簡介」「這不是政見？」時，走的是同一個提問流程（所以你會看到訪客自己寫的那句話），但任務型別不是 `question` 而是 `progress_stale`／`policy_missing`／`profile_gap`／`policy_validity`。看 `suggested_contribution_type` 就知道該交哪一種：那種任務要的是**改資料**，只回一段 `question_answer` 不算完成。
