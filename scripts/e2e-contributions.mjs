...
async function routeFeed(page, feedRequests) {
  await page.route('**/functions/v1/contributions-feed**', async (route) => {
    ...
    const items = fixture.items.filter((it) => (status === 'all' || it.status === status) && (!type || it.contribution_type === type) && (!agent || it.agent_name === agent))
...
try {
  ...
  const page = await context.newPage()
  await routeTasks(page)
  await page.route('**/functions/v1/contributions-feed**', (route) => route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ ...fixture, items: [], count: 0, has_more: false }) }))
  await page.goto(`http://localhost:${PORT}/ai-assistant`, { waitUntil: 'networkidle' })
