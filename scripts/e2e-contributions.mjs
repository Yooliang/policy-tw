...
try {
  for (const viewport of [{ width: 1280, height: 900, name: 'desktop' }, { width: 390, height: 844, name: 'mobile' }]) {
    ...
    // 近 7 日：預設含驗證（兩組柱子），取消勾選只剩提交
    const dailyToggle = page.locator('[data-testid="daily-include-verifications"]')
    await page.waitForSelector('aside .apexcharts-bar-series .apexcharts-series')
    check(await dailyToggle.isChecked(), `${viewport.name}: 近 7 日「含驗證」預設打勾`)
    check(await page.locator('aside .apexcharts-bar-series .apexcharts-series').count() === 2, `${viewport.name}: 含驗證時兩組柱子（提交、驗證）`)
    await dailyToggle.uncheck()
    await page.waitForFunction(() => document.querySelectorAll('aside .apexcharts-bar-series .apexcharts-series').length === 1)
    check(await page.locator('aside .apexcharts-bar-series .apexcharts-series').count() === 1, `${viewport.name}: 取消勾選只剩提交`)
    await dailyToggle.check()
