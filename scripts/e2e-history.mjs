// 查核履歷 Playwright 抽驗：政見頁／人物頁／分析頁的「查核履歷」區塊（預設收合、帶筆數、展開時間軸、再展開看驗證者與改動）、空狀態顯示來源、手機不破版。
...
try {
  for (const viewport of [{ width: 1280, height: 900, name: 'desktop' }, { width: 390, height: 844, name: 'mobile' }]) {
    ...
    check(await page.locator('[data-testid="history-body"]').count() === 0, `${viewport.name}: 預設收合`)
  ...
  const page = await context.newPage()
  await routeHistory(page, { ...fixture, total: 0, count: 0, entries: [], origin: { kind: 'imported', note: '早期由 AI 搜尋匯入，尚未經過貢獻流程；來源見下方網址', source_url: 'https://www.cec.gov.tw/bulletin.pdf' } })
  await page.goto(`http://localhost:${PORT}/policy/${policyId}`, { waitUntil: 'networkidle' })
  await page.waitForSelector('[data-testid="history-panel"]')
  check((await page.locator('[data-testid="history-panel"]').textContent()).includes('AI 搜尋匯入'), '收合時就顯示來源說明')
  await page.locator('[data-testid="history-toggle"]').click()
  await page.waitForSelector('[data-testid="history-empty"]')
  const empty = await page.locator('[data-testid="history-empty"]').textContent()
  check(empty.includes('尚未經過 AI 貢獻流程') && empty.includes('bulletin.pdf'), '空狀態列來源網址，不留空白')
  await context.close()
} finally {
  await browser.close()
  server.kill()
